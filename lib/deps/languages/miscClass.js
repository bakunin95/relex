...
var miscClass = module.exports = {
	analyze: function(files,cbmiscClass){
		miscClass.parsersNormal = _.sortBy(_.where(_.flatten(_.map(parsersList, function(n) { return n; })),{"active": true}, 'priority'));
		...
		async.eachLimit(files,5,function(parent,cbFiles){
			...
			async.eachSeries(miscClass.parsersNormal,function(parser,cbNormParse){
				...
				if(_.includes(parser.group,parent.group)){
					...
					setImmediate(function() { cbNormParse() });  
				}
			},function(err2){
				setImmediate(function() { cbFiles() });  
			});
		},function(){
			setImmediate(function() { cbmiscClass() }); 
		});
