...
var libJavaScript = module.exports = {
	    ...
	    findAssets: function(node,data,cbFindAssets){
	    	...
	    	async.parallel({
			    detective: function(cbdetective) {
			    	...
			    	try{
		    			...
		    			for (key in res) {						    
		    				if(res[key].substr(res.length - 3) !== ".js"){
		    					res[key] = res[key]+".js";
		    				}
						}
			        ...
			        cbdetective(null, res);
			    },
			    detectiveAmd: function(cbdetectiveamd) {
			    	var res = null;
			    	try{
			        ...
			        cbdetectiveamd(null, res);
			    },
			    ajax: function(cbAjax) {
			    	//try{		    		
				...
				libJavaScript.errorHandler(err);
				var union = _.union(results.detective,results.detectiveAmd,results.ajax);
				var assets = new Array();
				for (key in union) {
					assets.push({"name":union[key]});	
				}
						...
						if(ajaxResult.length>0){
							for (key in ajaxResult) {
								if(ajaxResult[key] !== null){
									//console.log("yyyyyyyyyyyyyyyyyiiiiiiiiiiiii",ajaxResult[key].arguments[0].properties[2].value.value);
									//if(_.isUndefined(ajaxResult[key].arguments[0].value) == false){
										assets.push(ajaxResult[key].arguments[0].properties[2].value.value);	   		
									//}
								}
							}
						}
